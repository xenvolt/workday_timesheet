...
const ManagerDashboard = () => {
    ...
    const handleCheckIn = async () => {
        try {
            ...
            const response = await fetch(
                "http://localhost:5000/api/manager/attendance/checkin",
                {
                    method: "POST",
    ...
    useEffect(() => {
        const handleUnload = () => {
            ...
            navigator.sendBeacon(
                "http://localhost:5000/api/manager/attendance/timeout",
                new Blob(
                    [JSON.stringify({ email: user.email, date, timeOut })],
    ...
    // Initial data fetch for leave and timesheet.
    useEffect(() => {
        if (!user?.email) return;

        fetch(`http://localhost:5000/api/manager/leave/email/${user.email}`)
            .then((res) => res.json())
            .then((data) => {
                ...
                setLeaveData({ approved, pending, rejected, total });
            })
            .catch((err) => console.error("Leave fetch error:", err));

        fetch(`http://localhost:5000/api/manager/timesheet/email/${user.email}`)
            .then((res) => res.json())
            .then((data) => {
                setTimesheetData(data); // This state holds the raw data from the DB
                setActivityLog(
                    data.map((item) => ({
                        date: item.date,
                        task: item.task,
                ...
                const formattedData = data
                    .filter((entry) => entry.hours !== undefined && entry.date)
                    .map((entry) => ({
                        date: entry.date,
    ...
    useEffect(() => {
        const today = new Date('2025-06-23T00:00:00');
        ...
        <div className="p-6 min-h-screen relative">
          ...
          `}</style>
                ...
                <div className="bg-gray-700 p-6 rounded-xl shadow-lg h-80">
                    ...
                    <div className="flex gap-20 justify-center items-center py-2 h-32">
                        <LeaveCard type="Annual" used={4} total={10} color="blue" />
                        <LeaveCard type="Sick" used={1} total={5} color="yellow" /></div>
                    <div className="flex gap-6 justify-center items-center py-2 h-16">
                        <LeaveCard type="Personal" used={1} total={4} color="red" /></div>
                </div>
            </div>
                    ...
                    <div className="flex justify-between items-center mb-4">
                        ...
                        <div className="flex items-center gap-2">
                            <button
                                ...
                                className="bg-blue-200 text-blue-800 px-3 py-1 rounded-md text-sm font-medium hover:bg-blue-300 transition-colors"
                            >
                                ← Back
                            </button>
